...
NOTIFY_BOT_SECRET = _env(
    "NOTIFY_BOT_SECRET", ""
)  # Railway 1'den gelen webhook güvenlik anahtarı
...
def check_subscription(user_id: int) -> dict:
    """Handle join request subscription check. Returns dict with 'active' key."""
    return {"active": _is_subscription_active(user_id)}
...
def set_subscription(
    user_id: int, plan: str, payment_id: str, tx_hash: str, usd_amount: float
) -> None:
    global access_data
    now = datetime.now()
    expires_at = (
        None
        if plan == "lifetime"
        else (now + timedelta(days=30)).strftime("%Y-%m-%d %H:%M:%S")
    )
    subs = access_data.get("subscriptions", {})
    subs[str(user_id)] = {
        "plan": plan,
...
def _derive_child(key: bytes, chain: bytes, index: int) -> tuple[bytes, bytes]:
    ...
    if index >= 0x80000000:
        ...
        pub = (
            _ecdsa.SigningKey.from_string(key, curve=_ecdsa.SECP256k1)
            .get_verifying_key()
    ...
    il, ir = I[:32], I[32:]
    child_key = (
        int.from_bytes(il, "big") + int.from_bytes(key, "big")
    ) % _ecdsa.SECP256k1.order
...
def _privkey_to_ltc_address(privkey: bytes) -> str:
    ...
    return (
        base64.b58encode_check(payload + checksum)
        if False
        else _b58encode(payload + checksum)
    )
...
def _privkey_to_sol_address(privkey_seed: bytes) -> str:
    ...
    Bx = (
        15112221349535807912866137220509078750507884956996801867914974660316018259358
        % P
    )
    By = (
        46316835694926478169428394003475163141307993866256225615783033011972563625925
        % P
    )
    Bz, Bt = 1, Bx * By % P
...
def init_wallet() -> bool:
    ...
    try:
        ...
        _master_seed_bytes = hashlib.pbkdf2_hmac(
            "sha512", normalized.encode(), b"mnemonic", 2048, 64
        )
        _wallet_ready = True
...
def create_activation_payment(
    user_id: int, coin: str, full_name: str, username: str, plan: str
) -> dict:
...
def get_ltc_price_usd() -> float:
    try:
        ...
        try:
            r = http_requests.get(
                "https://api.coinbase.com/v2/prices/LTC-USD/spot", timeout=8
            )
            ...
            for inst in (
                ...
                if isinstance(parsed, dict):
                    ...
                    if (
                        ...
                        and info.get("destination") == address
                    ):
...
def verify_ltc_tx(
    tx: dict, expected_address: str, expected_ltc: float
) -> tuple[bool, str]:
...
def verify_usdc_tx(
    tx: dict, expected_address: str, expected_usdc: float
) -> tuple[bool, str]:
    if is_tx_processed(tx.get("hash", "")):
        return False, "already processed"
...
def generate_qr_bytes(data: str) -> bytes | None:
    try:
        ...
        logger.warning(
            "generate_qr_bytes: qrcode library not installed — pip install qrcode[pil]"
        )
        return None
    except Exception as e:
...
def sub_payment_markup(plan: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(
                    "💵 Pay with USDC (Solana)", callback_data=f"subpay_{plan}_usdc"
                )
            ],
            [
                InlineKeyboardButton(
                    "🪙 Pay with LTC", callback_data=f"subpay_{plan}_ltc"
                )
            ],
...
async def send_payment_with_qr(
    target,
    coin: str,
    usd_amount: float,
    address: str,
    ...
    if coin == "LTC":
        ...
        ltc_line = (
            f"• Send exactly: {round(usd_amount / ltc_price, 6)} LTC\n"
            if ltc_price > 0
            else ""
        )
    else:
    ...
    try:
        if qr_bytes:
            ...
            await target.reply_photo(
                photo=BytesIO(qr_bytes), caption=caption, reply_markup=reply_markup
            )
            return
        else:
            logger.warning(
                f"send_payment_with_qr: no QR bytes for coin={coin} address={address[:20]}"
            )
    except Exception as e:
...
async def activate_user(
    user_id: int, payment_id: str, tx_hash: str, coin: str, usd_amount: float, bot
) -> None:
    global access_data
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    logger.info(
        f"[ACTIVATION] user={user_id} plan={plan} coin={coin} amount={usd_amount} tx={tx_hash}"
    )
        ...
        await bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "🌐 Open Dashboard", callback_data="web_login"
                        )
                    ],
                ]
            ),
...
async def check_activation_payments_job(context) -> None:
    ...
    usdc_acts = {
        pid: r for pid, r in to_check.items() if r["coin"] in ("USDC", "USDC_SOLANA")
    }
    ...
    for pid, rec in ltc_acts.items():
        ...
        for tx in txs:
            ...
            await activate_user(
                int(rec["user_id"]), pid, tx["hash"], "LTC", received_usd, context.bot
            )
            break
...
def create_deposit_payment(user_id: int, coin: str, usd_amount: float) -> dict:
    ...
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    expires = datetime.fromtimestamp(time.time() + PAYMENT_TIMEOUT_SECONDS).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    record = {
        "deposit_id": dep_id,
...
async def credit_deposit(
    user_id: int, dep_id: str, tx_hash: str, coin: str, usd_amount: float, bot
) -> None:
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.error(
                f"deposit_credit Railway error {resp.status_code}: {resp.text[:200]}"
            )
    except Exception as e:
        logger.error(f"deposit_credit request error: {e}")

    logger.info(
        f"[DEPOSIT] user={user_id} amount=${usd_amount} coin={coin} tx={tx_hash}"
    )
        ...
        await query.message.reply_html(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "✅ Check Status", callback_data="dep_check"
                        )
                    ],
                    [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")],
                ]
...
async def handle_deposit_confirm(update, context) -> None:
    ...
    try:
        ...
        await update.message.reply_text(
            f"⚠️ Could not generate address. Please try again.\nError: {e}"
        )
        return
        ...
        await query.message.reply_html(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "💰 New Deposit", callback_data="deposit_menu"
                        )
                    ],
                    [InlineKeyboardButton("🏠 Main Menu", callback_data="main_menu")],
                ]
            ...
            for tx in txs:
                ...
                if ok:
                    ...
                    await credit_deposit(
                        user_id, dep_id, tx["hash"], coin, received_usd, query.get_bot()
                    )
                    found = True
                    break
    else:
...
async def approvedlist_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    if not is_admin(update.effective_chat.id):
        return
    ...
    for uid in approved[:50]:
        ...
        expires = (
            sub.get("expires_at", "—")[:10] if sub.get("expires_at") else "lifetime"
        )
...
async def notify_waitlist_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    ...
    for r in rows:
        try:
            await context.bot.send_message(
                chat_id=r["user_id"], text=f"📢 PGSM Update\n\n{text}"
            )
            sent += 1
        except Exception:
            failed += 1
        await asyncio.sleep(0.05)
    await update.message.reply_text(
        f"Waitlist notified. Sent: {sent}, Failed: {failed}"
    )


async def messageuser_command(
    update: Update, context: ContextTypes.DEFAULT_TYPE
) -> None:
    if not is_admin(update.effective_chat.id):
        return
    ...
    try:
        await context.bot.send_message(
            chat_id=uid, text=f"📢 Message from admin:\n\n{text}"
        )
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if blocked_by_maintenance(chat_id) and query.data not in (
        "support_unregistered",
        "restricted_home",
    ):
        await query.message.reply_text(
            "🔧 Bot is under maintenance. Please try again later."
    ...
    if data == "support_unregistered":
        ...
        await query.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("❌ Cancel", callback_data="restricted_home")]]
            ),
        )
        return
    ...
    if data.startswith("subpay_"):
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton(
                                "✅ I Paid — Check Status",
                                callback_data="check_activation",
                            )
                        ],
                        [
                            InlineKeyboardButton(
                                "🔙 Back", callback_data="restricted_home"
                            )
                        ],
                    ]
                ),
        ...
        try:
            ...
            await query.message.reply_text(
                f"⚠️ Could not generate payment address.\nError: {e}\n\nPlease try again or contact support."
            )
            return
        address = rec["address"]
        ...
        await send_payment_with_qr(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            "✅ I Paid — Check Status", callback_data="check_activation"
                        )
                    ],
                    [InlineKeyboardButton("🔙 Back", callback_data="restricted_home")],
                ]
    ...
    if data == "check_activation":
        ...
        if not existing:
            ...
            return
        await query.answer(
            "Payment is being monitored automatically. You'll be notified once confirmed.",
            show_alert=True,
        )
        return
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user_id in support_mode_users:
        ...
        try:
            ...
            await update.message.reply_text(
                "Could not send support message. Please try again shortly."
            )
            return
        support_mode_users.discard(user_id)
...
def build_application() -> Application:
    ...
    t1 = threading.Thread(
        ...
        args=(check_activation_payments_job, 60, 30),
        daemon=True,
    )
    t1.start()
    ...
    t2 = threading.Thread(
        target=_run_job_thread, args=(check_deposits_expiry_job, 60, 45), daemon=True
    )
    t2.start()
    ...
    def _notify_stock():
        ...
        try:
            if _notify_loop and _notify_loop.is_running():
                ...
                logger.warning(
                    "notify_stock: event loop henüz hazır değil (bot başlıyor olabilir)"
                )
                return _jsonify(
                    {"error": "bot henüz hazır değil, kısa süre sonra tekrar deneyin"}
                ), 503
